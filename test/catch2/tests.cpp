...
{
    static constexpr auto bf = BitFieldConfiguration<0, 0, 4, uint8_t,
...
TEST_CASE("CustomRange with predicate checking multiple of 4")
{
    ...
    static constexpr auto field = FieldConfiguration<0, uint8_t,
        FieldRanges::CustomRange<uint8_t, decltype(isMultOf4)>{ isMultOf4 }>{};
...
{
    static constexpr auto statusField = FieldConfiguration<0, uint8_t>{};
    static constexpr auto rpmField = FieldConfiguration<1, uint16_t>{};
...
TEST_CASE("Message registry dispatches to correct handler")
{
    ...
    auto entry1 = MessageParser::MessageEntry<uint8_t, 0x01, std::function<void(std::span<const uint8_t>)>>{
        .parser = [&result1](std::span<const uint8_t> payload) { result1 = payload[0]; }
    };
    auto entry2 = MessageParser::MessageEntry<uint8_t, 0x02, std::function<void(std::span<const uint8_t>)>>{
        .parser = [&result2](std::span<const uint8_t> payload) { result2 = payload[0]; }
    };
...
{
    static constexpr auto f = NamedFieldConfiguration<0, uint8_t,
        FieldRanges::MinMaxRange{ .min = uint8_t{0}, .max = uint8_t{100} }>{ .name = "sensor" };
    constexpr std::array<uint8_t, 1> msg = { 50 };
